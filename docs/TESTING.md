...
2. AQE 2.3.0 with the tested ACO 1.6.2 release on both sides:
...
4. Reinstall the tested compatible ACO `1.6.2`:
...
Automated tests cover checked capacity arithmetic, malformed/canonical NBT, local fallback preservation, optional API reflection, shared host reservations, host save/load, host lifecycle ownership, and the 1.21.1 bytecode contract. They do not replace Forge/Arclight startup, transformed-Mixin, in-world structure, multiplayer, or crash-recovery tests.
