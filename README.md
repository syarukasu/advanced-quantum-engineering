...
AE2 crafting and synchronization optimization code is intentionally not part of this mod. It lives in the separate `ae2-crafting-optimizer` project so Quantum Computer block behavior and AE2 optimization behavior can be tested independently. Compatible ACO 1.5.9 releases are optional: AQE runs without ACO and uses its versioned BigInteger host API when it is present and enabled.
...
- AQE with compatible ACO `[1.5.9,1.6.0)` releases: AQE reflectively activates ACO BigInteger host API v3. Standard long jobs and exact BigInteger parent jobs share one physical capacity ledger. ACO divides a deterministic parent into recipe-specific checked-long child windows and keeps their reservations tied to the parent.
...
AQE contains no runtime ACO class reference. The optional adapter resolves and validates API v3 only after NeoForge reports a compatible ACO `[1.5.9,1.6.0)` release as loaded. ACO keeps exact BigInteger plan and inventory values in its own sidecars; AQE receives only the registered API v3 host and does not depend on those internal classes.
...
The generated jar is written under `build/libs/aqe2.2.6_1.21.1.jar`.
