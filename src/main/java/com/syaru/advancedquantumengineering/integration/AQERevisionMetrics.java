...
/** Revision再利用が働いていることを検証する低負荷カウンタ。 */
public final class AQERevisionMetrics {
    ...
    public static void recordStructureScan() {
        STRUCTURE_FULL_SCANS.increment();
    }

    public static void recordReservationRebuild() {
        RESERVATION_FULL_REBUILDS.increment();
    }

    public static void recordHostSnapshotRead() {
        HOST_SNAPSHOT_READS.increment();
    }

    public static void recordHostReconcile() {
        HOST_RECONCILES.increment();
    }

    public static void recordCoprocessorHotPathReuse() {
        COPROCESSOR_HOT_PATH_REUSES.increment();
    }

    public static void recordPresentationReuse() {
        PRESENTATION_REUSES.increment();
    }
